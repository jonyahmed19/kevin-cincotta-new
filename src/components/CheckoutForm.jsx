...
const Original = ({ series }) => (
  <>
    <p>
      My paintings are done in a style I call{" "}
      <i>family-friendly cubist psychedelia</i>.
    </p>
  </>
);
...
const Print = ({ painting }) => (
  <>
    <p>
      Archival quality prints on thick, lightly textured {GetGsm(painting)}gsm
      paper using pigment-based ink rather than dye-based inkjet ink.
    </p>
  </>
);
...
const getNote = (format, painting) => {
  ...
  switch (format.toLowerCase()) {
    case "original":
      return <Original series={series} />;
    case "xl":
      return <Print painting={painting} />;
    case "large":
      return <Print painting={painting} />;
    case "small":
      return <Print painting={painting} />;
...
export default function CheckoutForm({
  ...
  const checkout = async (e) => {
    ...
    console.log("CHECKOUT", import.meta.env.VITE_REACT_APP_CHECKOUT_URL);
    fetch(import.meta.env.VITE_REACT_APP_CHECKOUT_URL, {
      method: "POST",
  ...
  return (
    <div className="checkoutNote">
      ...
      {loadingStripe ? (
        ...
        <>
          <button className="buy" onClick={(e) => checkout(e)}>
            Purchase
          </button>
